...
export default function CartItem(product) {
    ...
    const [quantity, setQuantity] = useState(product.product.quantityToBuy)
    ...
    const handleDec = async () => {
        ...
        await updateQuantity(product.product._id, prodCount)
    };
    ...
    const handleRemove = async () => {
        await deleteCartItem(product.product._id,product)
    }
        ...
        <>
            ...
            <Box mb={3} mt={{ xs: 0, sm: 3 }} >
                ...
                <Typography variant="" color='#878787' fontSize={"12px"}>
                    <b style={{ color: "#0A0102" }}>Rs. {product.product.product_id.discountPrice} </b><del > Rs. {product.product.product_id.price}</del>
                </Typography>
